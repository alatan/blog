...
## 并发三要素
#### 可见性
由于CPU增加了缓存，以均衡与内存的速度差异导致，方法：一个线程对共享变量的修改，另外一个线程能够立刻看到。

#### 原子性
由于操作系统增加了进程、线程，以分时复用CPU，进而均衡CPU与I/O设备的速度差异导致，方法：一个操作或者多个操作 要么全部执行并且执行的过程不会被任何因素打断，要么就都不执行。
...
#### 有序性
由于编译程序指令重排序优化指令执行次序，使得缓存能够得到更加合理地利用导致，方法：程序执行的顺序按照代码的先后顺序执行。
