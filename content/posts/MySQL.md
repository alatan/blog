...
主键索引是聚集索引，在索引中保存了数据，从而避免直接读取磁盘，因此对查询性能有很大的提升。 
...
* 非聚集索引（non-clustered index），也叫辅助索引（普通索引）（secondary index）
* 聚集索引和非聚集索引都是B+树结构
...
InnoDB 的 B+Tree 索引分为主索引和辅助索引（普通索引）。 
...
### B+Tree的叶子节点存储内容不同分为聚集索引和非聚集索引

**聚集索引的叶子节点 data 域记录着完整的数据记录。聚集索引指的是表本身中数据按哪个字段的值来进行排序。因此，一个表只能包含一个聚簇索引，而且使用聚集索引不会付出额外IO开销。**
![](/images/db/rdb/PKIndex.jpg "聚集索引")

**辅助索引（普通索引）的叶子节点的 data 域记录着主键的值，因此在使用辅助索引（普通索引）进行查找时，需要先查找到主键值，然后使用主键在主索引上再进行对应的检索操作，这也就是所谓的“回表查询”，如果覆盖索引则无需再“回表查询”**
![](/images/db/rdb/fuzhuIndex.jpg "非聚集索引")

### 覆盖索引（covering index）
回表查询是由于非聚簇索引的 B+Tree 叶子节点只记录了，PK cols 与 key。这就造成当查询字段非主键跟索引字段时，索引树无法直接提供所有数据列，从而要进行一次回表查询，也就是拿着主键 ID 再进行一次查询操作。

指一个查询语句需要的结果在索引中就能够取得，不必从数据表中读取。也可以称之为实现了索引覆盖。 
...
* 索引下推在非主键索引上的优化，可以在有like条件查询的情况，可以有效减少回表的次数，大大提升了查询的效率。
...
* 幻读的重点在于新增或者删除：在同一事务中，同样的条件，第一次和第二次读出来的记录数量不一样。（因为中间有其他事务提交了插入/删除）
...
该锁是对索引记录进行加锁！锁是在加索引上而不是行上的。注意了，InnoDB 一定存在聚集索引，因此行锁最终都会落到聚集索引上！
